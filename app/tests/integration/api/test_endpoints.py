...
def test_sources_include_runtime_capability_sections() -> None:
    response = client.get("/api/sources")
    assert response.status_code == 200
    source = next(item for item in response.json()["sources"] if item["name"] == "gallica")

    assert "declared_capabilities" in source
    assert "effective_capabilities" in source
    assert "probe_status" in source
    assert source["probe_status"] in {"supported", "not_supported", "skipped", "timeout", "failed"}
    assert isinstance(source["supports_runtime_detection"], bool)
