...
1. **Secret** : `HF_TOKEN` (token Hugging Face avec droit d’écriture sur le Space). Le workflow pousse via authentification git `__token__`.
...
Le workflow tente d'abord un push normal, puis retente automatiquement en `--force-with-lease` si la synchro échoue (cas courant: historique Space non fast-forward / initial commit différent).


Diagnostic rapide si le Space affiche **"No application file"** :

1. vérifier dans **GitHub Actions** qu'un run `Deploy Hugging Face Space` a bien réussi ;
2. vérifier que la PR n'est pas en *draft* (les PR draft ne sont pas déployées) ;
3. vérifier que le secret repo s'appelle exactement `HF_TOKEN` et que le token a bien accès write au Space ;
4. vérifier que le token Hugging Face a bien le droit **write** sur `Ma-Ri-Ba-Ku/Clafoutis` ;
5. lancer manuellement le workflow via `workflow_dispatch` pour forcer une première synchro.
6. vérifier dans les logs que la validation token passe (`/api/whoami-v2`) puis que l'authentification git HF passe (`git ls-remote hf-space`).
7. en cas de non-fast-forward, vérifier que la tentative `--force-with-lease` est bien exécutée et aboutit.
